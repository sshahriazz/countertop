...
function BackgroundImageWithTitle({ item }: any) {
    ...
    return (
        <section className="relative">
            ...
            {item.img ? <div ref={element}>
                <div className="bg-black h-full w-full bg-opacity-50 absolute">
                </div>
                <div>
                    <Image
                        className="w-full snap-center h-screen object-cover"
                        src={item.img}
                        alt="Picture of the user"
                    />
                </div>
            </div>
                : <video autoPlay loop muted className="w-full h-screen object-cover snap-center">
